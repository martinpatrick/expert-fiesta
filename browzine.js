browzine.primo = (function() {
  ...
  function getPmid(scope) {
    var pmid = "";
    var result = getResult(scope);

    if (result && result.pnx) {
      if (result.pnx.addata && result.pnx.addata.pmid) {
        if (result.pnx.addata.pmid[0]) {
          pmid = result.pnx.addata.pmid[0].trim();
        }
      }
    }

    return encodeURIComponent(pmid);
  };
  ...
  function getEndpoint(scope) {
    ...
    if (isArticle(scope) && !getDoi(scope) && getPmid(scope)) {
      var pmid = getPmid(scope);
      endpoint = api + "/articles/pmid/" + pmid + "?include=journal,library";
    }

    if (isArticle(scope) && !getDoi(scope) && !getPmid(scope) && getIssn(scope)) {
      var issn = getIssn(scope);
      endpoint = api + "/search?issns=" + issn;
    }
